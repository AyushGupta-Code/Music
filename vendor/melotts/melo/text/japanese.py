...
def _resolve_mecab_args() -> str:
    """Pick a dictionary that ships with our deps so MeCab always finds one."""
    env_args = os.environ.get("MECAB_ARGS")
    if env_args:
        return env_args

    candidates = []
    # unidic-lite is bundled and works without extra downloads
    try:
        unidic_lite = importlib.import_module("unidic_lite")
        candidates.append(Path(unidic_lite.__path__[0]) / "dicdir")
    except ImportError:
        pass

    # Full unidic is optional but preferred when available.
    try:
        unidic = importlib.import_module("unidic")
        dic_dir = Path(getattr(unidic, "DICDIR", ""))
        candidates.append(dic_dir)
    except ImportError:
        pass

    for dic_dir in candidates:
        if dic_dir and dic_dir.exists():
            dic_dir = dic_dir.resolve()
            mecabrc = dic_dir / "mecabrc"
            if mecabrc.exists():
                os.environ.setdefault("MECABRC", str(mecabrc))
                return f"-r {mecabrc} -d {dic_dir}"
            dicrc = dic_dir / "dicrc"
            if dicrc.exists():
                os.environ.setdefault("MECABRC", str(dicrc))
            return f"-d {dic_dir}"
...
def _get_tagger():
    global _TAGGER
    if _TAGGER is None:
        _TAGGER = MeCab.Tagger(_resolve_mecab_args())
    return _TAGGER
